...
<?php 
if(isset($_GET['edit'])){
   $id_dv = mysqli_real_escape_string($conn,$_GET['edit']);
      $query = "SELECT * FROM doanvien WHERE id_dv = $id_dv";
      $query_run = mysqli_query($conn,$query);
      if(mysqli_num_rows($query_run) > 0){
        $dv = mysqli_fetch_array($query_run);
        // print_r($dv);
        ?>
<!-- FORM CẬP NHẬT -->
...
<form action="doanvien.php" method="POST"  enctype="multipart/form-data" style="background:#d5d1f6">
      ...
      <div class="col-6">
      <legend class="w-100 p-3 fs-1 text-center">Cập Nhật Thông Tin Đoàn Viên</legend>
      <?php include('message.php');?>
            <div class="mb-3">
              <input type="text" name ="id_dv" class="form-control" value ="<?php echo $dv['id_dv']?>">
            </div>
            <div class="mb-3">
              <label>Tên DV</label>
                <input type="text" class="form-control" value="<?php echo $dv['ten_dv']?>"  name ="TenDV" placeholder="NGUYỄN VĂN A">
            </div>
            <div class="mb-3">
              <label>Ngày Sinh</label>
                <input type="date" class="form-control" name ="NgaySinh" value="<?php echo $dv['ngay_sinh']?>" aria-hidden="Ngày Sinh:">
              </div>
              <div class="mb-3">
            ...
            <div class="mb-3">
              <label>Địa Chỉ</label>
                <input type="text" class="form-control"value="<?php echo $dv['dia_chi']?>" name="DiaChi" >
              </div>
            
              <div class="mb-3">
                <label>SĐT</label>
                <input type="text" class="form-control"value="<?php echo $dv['sdt']?>" name="SDT"  >
              </div>
          
              <div class="mb-3">
                <label>Chi đoàn</label>
                <input type="text" class="form-control"value="<?php echo $dv['chi_doan']?>" name="ChiDoan" >
              </div>
              ...
              <label >Ngày Vào Đoàn</label>
                <input type="date" class="form-control" name="NgayVao" value="<?php echo $dv['ngay_vao']?>" placeholder="01/01/1999">
              </div>
              ...
              <div class="mb-3">
              <button type="submit" name="capnhatDV" class="btn btn-primary">Cập nhật</button>
                <a href="createDV.php" class="btn btn-primary">Trở về</a>
            </div>
          </div>
        </div>
      </div>
